...
const AppointmentPanel: React.FC<AppointmentPanelProps> = ({ role }) => {
  ...
  const markCompleted = async (id: string) => {
    if (!confirm('İşlemi tamamlandı olarak işaretlemek istediğinizden emin misiniz?')) return;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {['all','pending','confirmed','completed','cancelled'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? 'Tümü' : f === 'pending' ? 'Bekleyen' : f === 'confirmed' ? 'Onaylı' : f === 'completed' ? 'Tamamlanan' : 'İptal'}
            </button>
          ))}
        </div>
      </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {apt.reservation_type === 'guaranteed' && (
                          <span className="flex items-center gap-1 text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full">
                            <Shield className="w-3 h-3" /> Estyi Güvenceli
                          </span>
                        )}
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-5">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-700">
                          {role === 'user' ? <><Building className="w-4 h-4 text-gray-400" />{apt.clinic_name || 'Klinik'}</> : <><User className="w-4 h-4 text-gray-400" />{apt.user_email || 'Hasta'}</>}
                        </div>
                        {apt.note && <div className="text-sm text-gray-500 italic">📝 {apt.note}</div>}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 ml-4">
                      <button onClick={() => goToChat(apt.conversation_id)} className="p-2 text-purple-600 hover:bg-purple-100 rounded-lg" title="Sohbet"><MessageCircle className="w-4 h-4" /></button>
                      {canRespond && <>
                        <button onClick={() => updateStatus(apt.id, 'confirmed')} className="p-2 text-green-600 hover:bg-green-100 rounded-lg" title="Onayla"><Check className="w-4 h-4" /></button>
                        <button onClick={() => updateStatus(apt.id, 'rejected')} className="p-2 text-red-600 hover:bg-red-100 rounded-lg" title="Reddet"><X className="w-4 h-4" /></button>
                      </>}
                      {canCancel && <button onClick={() => updateStatus(apt.id, 'cancelled')} className="p-2 text-gray-500 hover:bg-gray-200 rounded-lg" title="İptal"><Ban className="w-4 h-4" /></button>}
                      {role === 'clinic' && apt.status === 'confirmed' && !apt.treatment_completed && (
                        <button onClick={() => markCompleted(apt.id)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-lg" title="İşlem Tamamlandı"><Check className="w-4 h-4" /></button>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {completed.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Tamamlanan İşlemler</h3>
          <div className="space-y-3">
            {completed.map(apt => (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-sm font-semibold text-blue-700">İşlem Tamamlandı</span>
                      {apt.reservation_type === 'guaranteed' && (
                        <span className="flex items-center gap-1 text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full"><Shield className="w-3 h-3" /> Güvenceli</span>
                      )}
                      {apt.has_review && <span className="flex items-center gap-1 text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full"><Star className="w-3 h-3" /> Değerlendirildi</span>}
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 ml-5">
                  ...
                  <div className="flex items-center gap-1 ml-4">
                    ...
                    {role === 'user' && apt.reservation_type === 'guaranteed' && !apt.has_review && (
                      <button onClick={() => setReviewingApt(apt.id)} className="px-3 py-1.5 bg-yellow-500 text-white text-xs font-medium rounded-lg hover:bg-yellow-600">Yorum Yap</button>
                    )}
                  </div>
                </div>
                {reviewingApt === apt.id && (
      ...
      {past.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Geçmiş / İptal</h3>
          <div className="space-y-3">
            {past.map(apt => {
              const cfg = statusCfg[apt.status] || statusCfg.pending;
              return (
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
          ...
          <h3 className="text-lg font-medium text-gray-500">Randevu Bulunamadı</h3>
