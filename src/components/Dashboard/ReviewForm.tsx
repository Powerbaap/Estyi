...
const ReviewForm: React.FC<ReviewFormProps> = ({ appointmentId, clinicId, conversationId, onDone }) => {
  ...
  const handleSubmit = async () => {
    if (rating === 0) { alert('Lütfen bir puan seçin.'); return; }
    if (!user?.id) return;
    setSending(true);
    try {
  ...
  return (
    <div className="mt-4 p-4 bg-white rounded-xl border border-yellow-200 shadow-sm">
      <h4 className="text-sm font-semibold text-gray-900 mb-3">Deneyiminizi Değerlendirin</h4>
      ...
      <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Deneyiminizi anlatın (opsiyonel)" rows={3}
      ...
      <button onClick={handleSubmit} disabled={rating === 0 || sending}
        ...
        <Send className="w-4 h-4" />{sending ? 'Gönderiliyor...' : 'Yorumu Gönder'}
      </button>
    </div>
  );
};
